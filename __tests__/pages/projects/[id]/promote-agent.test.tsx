import React from 'react'
import { render, screen, waitFor, fireEvent } from '@testing-library/react'
import { useRouter } from 'next/router'
import { useSession } from 'next-auth/react'
import PromoteAgentPage from '@/pages/projects/[id]/promote-agent'
import { mockRouter, mockSession, mockFetchSuccess, mockFetchError } from '@/__tests__/utils/test-utils'

jest.mock('next/router', () => ({
  useRouter: jest.fn(),
}))

jest.mock('next-auth/react', () => ({
  useSession: jest.fn(),
}))

const mockProject = {
  id: 'project-123',
  name: 'Test Project Alpha',
  description: 'A premium residential project',
}

describe('Promote Agent Page', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(useRouter as jest.Mock).mockReturnValue(mockRouter)
    ;(useSession as jest.Mock).mockReturnValue({
      data: mockSession,
      status: 'authenticated',
    })
  })

  describe('Authentication', () => {
    it('redirects unauthenticated users to login', () => {
      ;(useSession as jest.Mock).mockReturnValue({
        data: null,
        status: 'unauthenticated',
      })

      render(<PromoteAgentPage project={mockProject} />)

      expect(mockRouter.push).toHaveBeenCalledWith('/login')
    })

    it('renders page for authenticated users', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/Add Yourself as Verified Agent/i)).toBeInTheDocument()
      })
    })
  })

  describe('Project Not Found', () => {
    it('displays not found message when project is null', () => {
      render(<PromoteAgentPage project={null} />)

      expect(screen.getByText('Project Not Found')).toBeInTheDocument()
    })

    it('shows back to projects link when project not found', () => {
      render(<PromoteAgentPage project={null} />)

      const link = screen.getByText('Back to Projects')
      expect(link).toHaveAttribute('href', '/projects')
    })
  })

  describe('Page Rendering', () => {
    it('renders page title', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/Add Yourself as Verified Agent/i)).toBeInTheDocument()
      })
    })

    it('displays project name', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(`Project: ${mockProject.name}`)).toBeInTheDocument()
      })
    })

    it('renders back to project link', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const link = screen.getByText('← Back to Project')
        expect(link).toHaveAttribute('href', `/projects/${mockProject.id}`)
      })
    })
  })

  describe('Verified Agent Benefits Section', () => {
    it('displays benefits info box', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Verified Agent Benefits')).toBeInTheDocument()
      })
    })

    it('explains verification badge benefit', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/blue verification badge/i)).toBeInTheDocument()
      })
    })

    it('displays verification icon', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const svg = screen.getByText('Verified Agent Benefits').closest('div')?.querySelector('svg')
        expect(svg).toBeInTheDocument()
      })
    })
  })

  describe('Duration Selection', () => {
    it('renders duration input', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        expect(input).toBeInTheDocument()
      })
    })

    it('marks duration as required', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const label = screen.getByText(/Promotion Duration/i)
        const requiredAsterisk = label.querySelector('.text-red-500')
        expect(requiredAsterisk).toBeInTheDocument()
      })
    })

    it('defaults duration to 30 days', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i) as HTMLInputElement
        expect(input.value).toBe('30')
      })
    })

    it('allows changing duration', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '60' } })
        expect(input).toHaveValue(60)
      })
    })

    it('sets minimum duration to 1', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        expect(input).toHaveAttribute('min', '1')
      })
    })

    it('sets maximum duration to 365', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        expect(input).toHaveAttribute('max', '365')
      })
    })

    it('shows helper text for duration', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/Choose how long you want to be featured/i)).toBeInTheDocument()
      })
    })
  })

  describe('Summary Box', () => {
    it('renders summary section', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Summary')).toBeInTheDocument()
      })
    })

    it('displays start date', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Start Date:')).toBeInTheDocument()
      })
    })

    it('displays duration', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Duration:')).toBeInTheDocument()
        expect(screen.getByText('30 days')).toBeInTheDocument()
      })
    })

    it('displays expiry date', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Expiry Date:')).toBeInTheDocument()
      })
    })

    it('displays total amount', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText('Total Amount:')).toBeInTheDocument()
        expect(screen.getByText('₹0')).toBeInTheDocument()
      })
    })

    it('shows pre-launch offer text', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/Pre-launch offer - Free!/i)).toBeInTheDocument()
      })
    })

    it('updates duration in summary when changed', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '90' } })

        expect(screen.getByText('90 days')).toBeInTheDocument()
      })
    })

    it('updates expiry date when duration changes', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '10' } })

        // Expiry date should update (tested by presence of text, exact date would be dynamic)
        expect(screen.getByText('Expiry Date:')).toBeInTheDocument()
      })
    })
  })

  describe('Purchase Button', () => {
    it('renders purchase button', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/Complete Purchase - Get Verified/i)).toBeInTheDocument()
      })
    })

    it('enables purchase button by default', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        expect(button).not.toBeDisabled()
      })
    })

    it('disables button when duration is less than 1', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '0' } })

        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        expect(button).toBeDisabled()
      })
    })

    it('displays verification icon on button', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        const svg = button.querySelector('svg')
        expect(svg).toBeInTheDocument()
      })
    })
  })

  describe('Purchase Process', () => {
    it('shows loading state during purchase', async () => {
      global.fetch = jest.fn(() =>
        new Promise(resolve => {
          setTimeout(() => {
            resolve({
              ok: true,
              json: () => Promise.resolve({}),
            } as Response)
          }, 100)
        })
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)

        expect(screen.getByText('Processing...')).toBeInTheDocument()
      })
    })

    it('disables button during purchase', async () => {
      global.fetch = jest.fn(() =>
        new Promise(resolve => {
          setTimeout(() => {
            resolve({
              ok: true,
              json: () => Promise.resolve({}),
            } as Response)
          }, 100)
        })
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)

        expect(button).toBeDisabled()
      })
    })

    it('shows spinner during processing', async () => {
      global.fetch = jest.fn(() =>
        new Promise(resolve => {
          setTimeout(() => {
            resolve({
              ok: true,
              json: () => Promise.resolve({}),
            } as Response)
          }, 100)
        })
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)

        const spinner = button.querySelector('.animate-spin')
        expect(spinner).toBeInTheDocument()
      })
    })

    it('sends duration to API', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '45' } })

        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        const callArgs = (global.fetch as jest.Mock).mock.calls[0]
        const body = JSON.parse(callArgs[1].body)
        expect(body.duration).toBe(45)
      })
    })

    it('redirects to project page after successful purchase', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        expect(mockRouter.push).toHaveBeenCalledWith(`/projects/${mockProject.id}`)
      })
    })

    it('handles purchase errors', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: false,
          json: () => Promise.resolve({ message: 'Payment failed' }),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        expect(mockRouter.push).not.toHaveBeenCalled()
      })
    })

    it('requires authentication for purchase', async () => {
      ;(useSession as jest.Mock).mockReturnValue({
        data: { user: { email: null } },
        status: 'authenticated',
      })

      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)

        expect(global.fetch).not.toHaveBeenCalled()
      })
    })
  })

  describe('Terms and Conditions', () => {
    it('displays terms text', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        expect(screen.getByText(/By completing this purchase, you agree/i)).toBeInTheDocument()
      })
    })
  })

  describe('Date Formatting', () => {
    it('displays dates in Indian format', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        // Should use en-IN locale (day, month name, year)
        const summaryBox = screen.getByText('Summary').closest('div')
        expect(summaryBox).toBeInTheDocument()
      })
    })
  })

  describe('API Integration', () => {
    it('calls correct API endpoint', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        const callArgs = (global.fetch as jest.Mock).mock.calls[0]
        expect(callArgs[0]).toBe(`/api/projects/${mockProject.id}/promote-agent`)
      })
    })

    it('uses POST method', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        const callArgs = (global.fetch as jest.Mock).mock.calls[0]
        expect(callArgs[1].method).toBe('POST')
      })
    })

    it('sends JSON content type', async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve({}),
        } as Response)
      ) as jest.Mock

      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const button = screen.getByText(/Complete Purchase - Get Verified/i)
        fireEvent.click(button)
      })

      await waitFor(() => {
        const callArgs = (global.fetch as jest.Mock).mock.calls[0]
        expect(callArgs[1].headers['Content-Type']).toBe('application/json')
      })
    })
  })

  describe('Expiry Date Calculation', () => {
    it('calculates expiry date correctly for 30 days', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const today = new Date()
        const expectedExpiry = new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000)

        // Just verify the expiry date section exists
        expect(screen.getByText('Expiry Date:')).toBeInTheDocument()
      })
    })

    it('recalculates expiry when duration changes', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const input = screen.getByPlaceholderText(/Enter number of days/i)
        fireEvent.change(input, { target: { value: '60' } })

        // Verify expiry date is still displayed after change
        expect(screen.getByText('Expiry Date:')).toBeInTheDocument()
      })
    })
  })

  describe('Visual Styling', () => {
    it('applies correct styling to info box', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const infoBox = screen.getByText('Verified Agent Benefits').closest('div')
        expect(infoBox?.className).toContain('bg-blue-50')
      })
    })

    it('displays verification icon in blue', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const icon = screen.getByText('Verified Agent Benefits').closest('div')?.querySelector('svg')
        expect(icon?.className).toContain('text-blue-600')
      })
    })

    it('displays amount in green', async () => {
      render(<PromoteAgentPage project={mockProject} />)

      await waitFor(() => {
        const amount = screen.getByText('₹0')
        expect(amount.className).toContain('text-green-600')
      })
    })
  })
})
