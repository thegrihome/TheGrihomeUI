...
import handler from '@/pages/api/user/verify-email'
...
describe('/api/user/verify-email', () => {
  ...
  describe('Email Verification Update', () => {
    beforeEach(() => {
      req.method = 'POST'
      req.body = { otp: '123456' }
      ;(getServerSession as jest.Mock).mockResolvedValue({
    ...
    it('should update user with emailVerified date', async () => {
      ...
      expect(emailVerifiedDate.getTime()).toBeGreaterThanOrEqual(beforeUpdate)
      expect(emailVerifiedDate.getTime()).toBeLessThanOrEqual(afterUpdate)
    })
    ...
    it('should set emailVerified to current date', async () => {
      ...
      expect(emailVerifiedDate).toBeInstanceOf(Date)
    })
  ...
  describe('Response Format', () => {
    ...
    it('should not return emailVerified date in response', async () => {
      await handler(req as NextApiRequest, res as NextApiResponse)

      const callArg = jsonMock.mock.calls[0][0]
      expect(callArg).not.toHaveProperty('emailVerified')
    })
